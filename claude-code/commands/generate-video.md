...
- **Family exists only as partner/API** (no matching OSS template, node category starts with `partner/`): route directly. Try `partner_generate` first — its description names providers, not model ids, so pass `type: "video"` plus your best-guess model slug (a wrong one bounces back naming the closest real slugs — retry once with the closest match before giving up), `prompt`, and any optional fields (`aspect_ratio`/`seed`/`duration`/`medias[]`); guessing costs one call, so do not skip the tool for want of a list. On success, return the artifact URL(s) and **stop** — do NOT continue with the workflow steps below. If `partner_generate` returns "unknown model" with no close match named, or "not yet implemented", continue to Step 1.
